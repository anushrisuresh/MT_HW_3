...
optparser.add_option("-s", "--stack-size", dest="s",
                     default=1, type="int",
...
sys.stderr.write("Decoding %s with Finite-State Algorithm...\n" % (opts.input,))
for f in french:
    hypothesis = namedtuple(
        "hypothesis", "logprob, lm_state, predecessor, phrase")
    initial_hypothesis = hypothesis(0.0, lm.begin(), None, None)

    # stacks is a list where each entry is a dict mapping lm_state to hypothesis
    stacks = [{} for _ in range(len(f) + 1)]
    stacks[0][lm.begin()] = initial_hypothesis

    for i in range(len(f)):
        for h in sorted(stacks[i].values(), key=lambda h: -h.logprob)[:opts.s]:  # Prune to top opts.s hypotheses
            for j in range(i + 1, len(f) + 1):
                if f[i:j] in tm:
                    for phrase in tm[f[i:j]]:
                        ...
                        # Store the best hypothesis for this lm_state at position j
